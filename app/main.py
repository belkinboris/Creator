...
def _example_link(text: str, purpose: str = "business") -> str:
    ...
    оптику. G4 (PRODUCT_ROADMAP, разбор соцплан.рф): пока пример собран не
    под ТУ аудиторию, что сейчас смотрит на страницу, ссылка молчала вовсе
    -- то есть человек не видел ПРИМЕРА НИКАКОГО, хотя настоящий готовый
    отчёт уже существует. Владелец отдельно называл отсутствие примера
    блокером доверия (см. /example) -- молчание в этом случае то же самое,
    что полное отсутствие. Честнее показать реальный отчёт с оговоркой, что
    он под другую задачу (принцип 4 не нарушен: не выдаём его молча ЗА
    оптику посетителя, прямо говорим об обратном), чем не показать ничего.
    """
    with Session(engine) as s:
    ...
    return (f'<a href="/example">{html.escape(text)}</a>'
            ' <span class="example-mismatch">(собран для другой задачи, но настоящий)</span>')
